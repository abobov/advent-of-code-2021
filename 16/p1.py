...
class Reader:
    def __init__(self, data: str):
        self.data = ''.join(["{0:04b}".format(int(x, 16)) for x in data])
        self.cursor = 0

    def get_position(self):
        return self.cursor

    def has_next(self, count=1) -> bool:
        return self.cursor + count < len(self.data)

    def next(self, count=1) -> str:
        chunk = self.data[self.cursor:self.cursor + count]
        self.cursor += count
        return chunk

    def next_int(self, count=1) -> int:
        return int(self.next(count), 2)
...
class OperatorPacket(Packet):
    ...
    def __init__(self, version: int, type_id: int, reader: Reader):
        super().__init__(version, type_id)
        self.sub_packets = self.read_data(reader)

    @staticmethod
    def read_data(reader):
        if reader.next_int() == 0:
            length = reader.next_int(15)
            start = reader.get_position()
            while reader.get_position() < start + length:
                yield parse_packet(reader)
        else:
            for _ in range(reader.next_int(11)):
                yield parse_packet(reader)

    def sum_of_versions(self) -> int:
        result = super().sum_of_versions()
        for packet in self.sub_packets:
            result += packet.sum_of_versions()
        return result
    ...
    def evaluate(self) -> int:
        ...
        if self.type_id == 0:
            ...
            return int(functools.reduce(operator.gt, values))
        elif self.type_id == 6:
            return int(functools.reduce(operator.lt, values))
        elif self.type_id == 7:
            return int(functools.reduce(operator.eq, values))


class LiteralPacket(Packet):
    def __init__(self, version: int, type_id: int, reader: Reader):
        super().__init__(version, type_id)
        self.data = self.read_data(reader)

    @staticmethod
    def read_data(reader) -> int:
        tmp = ''
        while reader.has_next():
            group = reader.next_int()
            tmp += reader.next(4)
            if group == 0:
                break
        return int(tmp, 2)
...
def parse_packet(reader: Reader) -> Packet:
    version = reader.next_int(3)
    type_id = reader.next_int(3)
    if type_id == 4:
        packet_class = LiteralPacket
    else:
        packet_class = OperatorPacket
    return packet_class(version, type_id, reader)
