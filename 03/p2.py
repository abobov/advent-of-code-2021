...
0 and 1 are equally common, keep values with a 1 in the position being
considered.
...
each). So, to find the oxygen generator rating, keep the number with a 1 in that
position: 10111.
...
There are fewer 0 bits (5) than 1 bits (7), so keep only the 5 numbers with
a 0 in the first position: 00100, 01111, 00111, 00010, and 01010.
...
class TreeNode:
    ...
    def add_child_line(self, bits):
        ...
        if child is None:
            ...
            child.count += 1
        child.add_child_line(bits[1:])

    def solve(self, comp):
        child = None
        for variant in filter(lambda x: x is not None, self.children):
            if child is None:
                child = variant
            elif comp(variant.count, child.count):
                child = variant
        if child is None:
            return ''
        return child.value + child.solve(comp)
...
def main():
    ...
    with open('input.txt') as fd:
        for line in fd:
            ...
            tree.add_child_line(striped_line)

    oxygen = int(tree.solve(lambda a, b: a >= b), 2)
    co2 = int(tree.solve(lambda a, b: a < b), 2)
    print(oxygen * co2)
