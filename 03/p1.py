...
The most common seco!nd bit of the numbers in the diagnostic report is 0, so the
...
def to_int(result, comp):
    return int(''.join('0' if comp(x) else '1' for x in result), base=2)
...
def main():
    with open('input.txt') as fd:
        ...
        for line in fd:
            ...
            if result is None:
                result = [0] * len(striped_line)
            for index, bit in enumerate(striped_line):
                result[index] += 1 if bit == '0' else -1

    gamma = to_int(result, lambda x: x > 0)
    epsilon = to_int(result, lambda x: x < 0)
    print(gamma * epsilon)
