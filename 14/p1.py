...
def cached(func):
    ...
    def wrapper(left, right, pairs, steps):
        key = hash((left, right, steps))
        if key not in cache:
            cache[key] = func(left, right, pairs, steps)
        return cache[key]
...
@cached
def solve(left, right, pairs, steps):
    if steps == 0:
        return EMPTY_COUNTER
    element = pairs[left + right]
    first = solve(left, element, pairs, steps - 1)
    second = solve(element, right, pairs, steps - 1)
...
def main(steps):
    with open('sample.txt') as fd:
        template, pairs = read_input(fd)
        counter = Counter(template)
        for i in range(len(template) - 1):
            counter += solve(template[i], template[i + 1], pairs, steps)
