...
with random dots and includes instructions on how to fold it up (your
...
right. The second value, y, increases downward. So, the coordinate 3,0
is to the right of 0,0, and the coordinate 0,7 is below 0,0. The
coordinates in this example form the following pattern, where # is a dot
...
def fold(dots, fold_direction, fold_line):
    if fold_direction == 'fold along y':
        for x, y in set(dots):
            if y > fold_line:
                dots.remove((x, y))
                dots.add((x, (2 * fold_line - y)))
    else:
        for x, y in set(dots):
            if x > fold_line:
                dots.remove((x, y))
                dots.add(((2 * fold_line - x), y))
