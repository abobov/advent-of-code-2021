...
def solve(position, target):
    return sum(1 * (x + 1) for x in range(abs(position - target)))


def main():
    with open('input.txt') as fd:
        positions = [int(x) for x in fd.readline().strip().split(',')]
        target = math.floor(sum(positions) / len(positions))
        print(sum([solve(x, target) for x in positions]))
