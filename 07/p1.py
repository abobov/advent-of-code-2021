...
them otherwise) zooms in to rescue you! They seem to be preparing to blast
a hole in the ocean floor; sensors indicate a massive underground cave system
...
def main():
    with open('input.txt') as fd:
        positions = [int(x) for x in fd.readline().strip().split(',')]
        positions.sort()
        median = positions[math.floor(len(positions) / 2)]
