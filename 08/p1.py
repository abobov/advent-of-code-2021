...
digit that uses three segments, dab in the above example means that to render
a 7, signal lines d, a, and b are on. Because 4 is the only digit that uses four
...
def main():
    with open('input.txt') as fd:
        result = 0
        for line in fd:
            _, message = line.strip().split(' | ')
            result += len(list(filter(lambda x: x in [2, 3, 4, 7], [len(x) for x in message.split(' ')])))
        print(result)
